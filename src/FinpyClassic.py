...
def __Check_JDate_Validity__(date, key_word):
    try:
        if (len(date.split('-')[0]) == 4):
            date = jdatetime.date(year=int(date.split('-')[0]), month=int(date.split('-')[1]),
...
def get_tse_webid(stock: str = 'پترول') -> pd.DataFrame:
    ...
    def srch_req(srch_key):
        srch_page = requests.get(f'http://cdn.tsetmc.com/api/Instrument/GetInstrumentSearch/{srch_key}',
                                 headers=headers)
        ...
        srch_res = pd.DataFrame(srch_res[['Name', 'WebID', 'NameSplit', 'SymbolSplit', 'Market']].values,
                                columns=['Name', 'WebID',
                                         'NameSplit', 'SymbolSplit', 'Market'],
    ...
    if len(df_symbol) > 0:
        ...
        df_symbol['Market'] = df_symbol['Market'].map(
            {'N': 'بورس', 'Z': 'فرابورس', 'D': 'فرابورس', 'A': 'پایه زرد', 'P': 'پایه زرد', 'C': 'پایه نارنجی',
             'L': 'پایه قرمز',
...
def __Get_TSE_WebID__(stock):
    ...
    def request(name):
        ...
        data = pd.DataFrame(data[['Name', 'WEB-ID', 'Name-Split', 'Symbol-Split', 'Market']].values,
                            columns=['Name', 'WEB-ID',
                                     'Name-Split', 'Symbol-Split', 'Market'],
...
def __Get_TSE_Sector_WebID__(sector_name):
    sector_list = ['زراعت', 'ذغال سنگ', 'کانی فلزی', 'سایر معادن', 'منسوجات', 'محصولات چرمی', 'محصولات چوبی',
                   'محصولات کاغذی', 'انتشار و چاپ', 'فرآورده های نفتی', 'لاستیک', \
                   'فلزات اساسی', 'محصولات فلزی', 'ماشین آلات', 'دستگاه های برقی', 'وسایل ارتباطی', 'خودرو',
    ...
    sector_web_id = [34408080767216529, 19219679288446732, 13235969998952202, 62691002126902464, 59288237226302898,
                     69306841376553334, 58440550086834602, 30106839080444358, 25766336681098389, \
                     12331083953323969, 36469751685735891, 32453344048876642, 1123534346391630, 11451389074113298,
                     33878047680249697, 24733701189547084, 20213770409093165, 21948907150049163, 40355846462826897, \
                     54843635503648458, 15508900928481581, 3615666621538524, 33626672012415176, 65986638607018835,
                     57616105980228781, 70077233737515808, 14651627750314021, 34295935482222451, 72002976013856737, \
                     25163959460949732, 24187097921483699, 41867092385281437, 61247168213690670, 61985386521682984,
                     4654922806626448, 8900726085939949, 18780171241610744, 47233872677452574, 65675836323214668, \
                     59105676994811497]
...
def get_price_history(stock: str = 'خودرو',
    ...
    def get_price_data(ticker_no, ticker, name, market):
        r = requests.get(f'http://cdn.tsetmc.com/api/ClosingPrice/GetClosingPriceDailyList/{ticker_no}/0',
                         headers=headers)
        df_history = pd.DataFrame(r.json()['closingPriceDaily'])
        columns = ['Date', 'High', 'Low', 'Final', 'Close', 'Open', 'Y-Final', 'Value', 'Volume', 'No']
        df_history = df_history[
            ['dEven', 'priceMax', 'priceMin', 'pClosing', 'pDrCotVal', 'priceFirst', 'priceYesterday', 'qTotCap',
             'qTotTran5J', 'zTotTran']]
    ...
    if (not ignore_date):
        ...
        if (end_date == None):
            return
        start = jdatetime.date(year=int(start_date.split('-')[0]), month=int(start_date.split('-')[1]),
                               day=int(start_date.split('-')[2]))
        end = jdatetime.date(year=int(end_date.split('-')[0]), month=int(end_date.split('-')[1]),
    ...
    df_history = pd.DataFrame({},
                              columns=['Date', 'High', 'Low', 'Final', 'Close', 'Open', 'Y-Final', 'Value', 'Volume',
                                       'No', 'Ticker', 'Name', 'Market']).set_index('Date')

    # loop to get data from different pages of a ticker:
    for index, row in (ticker_no_df.reset_index()).iterrows():
        try:
            df_temp = get_price_data(ticker_no=row['WebID'], ticker=row['Ticker'], name=row['Name'],
                                     market=row['Market'])
    ...
    df_history = df_history[
        ['Date', 'Weekday', 'Y-Final', 'Open', 'High', 'Low', 'Close', 'Final', 'Volume', 'Value', 'No', 'Ticker',
         'Name', 'Market']]
...
def Get_Price_History(stock='خودرو', start_date='1400-01-01', end_date='1401-01-01', ignore_date=False,
                      adjust_price=False, show_weekday=False, double_date=False):
    """
    ...
    def get_price_data(ticker_no, ticker, name, data_part):
        r = requests.get(f'http://old.tsetmc.com/tsev2/data/InstTradeHistory.aspx?i={ticker_no}&Top=999999&A=0',
                         headers=headers)
    ...
    for index, row in (ticker_no_df.reset_index()).iterrows():
        try:
            df_temp = get_price_data(ticker_no=row['WEB-ID'], ticker=row['Ticker'], name=row['Name'],
    ...
    for col in cols:
        df_history[col] = df_history[col].apply(
            lambda x: int(x))  # convert to int because we do not have less than Rial
    # --------------------------------------------------------------------------------------------------------------------
    # Adjust price data:
    if (adjust_price):
        df_history['COEF'] = (df_history['Y-Final'].shift(-1) / df_history['Final']).fillna(1.0)
...
def get_ri_history(stock: str = 'خودرو',
    ...
    * All data are taken from the new website of Tehran Stock Exchange.
    ...
    def get_ri_data(ticker_no, ticker, name, market):
        ...
        cols = ['Date', 'WebID', 'Vol_Buy_R', 'Vol_Buy_I', 'Val_Buy_R', 'Val_Buy_I', 'No_Buy_I', 'Vol_Sell_R',
                'No_Buy_R', 'Vol_Sell_I', 'Val_Sell_R', 'Val_Sell_I', 'No_Sell_I', 'No_Sell_R']
        df_RI_tab.columns = cols
        df_RI_tab = df_RI_tab[
            ['Date', 'No_Buy_R', 'No_Buy_I', 'No_Sell_R', 'No_Sell_I', 'Vol_Buy_R', 'Vol_Buy_I', 'Vol_Sell_R',
             'Vol_Sell_I', 'Val_Buy_R', 'Val_Buy_I', 'Val_Sell_R', 'Val_Sell_I']]
    ...
    # create an empty dataframe:   
    df_RI_tab = pd.DataFrame({}, columns=['Date', 'No_Buy_R', 'No_Buy_I', 'No_Sell_R', 'No_Sell_I', 'Vol_Buy_R',
                                          'Vol_Buy_I', 'Vol_Sell_R',
                                          'Vol_Sell_I', 'Val_Buy_R', 'Val_Buy_I', 'Val_Sell_R', 'Val_Sell_I', 'Ticker',
                                          'Name', 'Market']).set_index('Date')
...
def Get_RI_History(stock='خودرو', start_date='1400-01-01', end_date='1401-01-01', ignore_date=False,
        ...
        if (alt):
            ...
            df_RI_tab = df_RI_tab[
                ['Date', 'No_Buy_R', 'No_Buy_I', 'No_Sell_R', 'No_Sell_I', 'Vol_Buy_R', 'Vol_Buy_I', 'Vol_Sell_R',
                 'Vol_Sell_I', 'Val_Buy_R', 'Val_Buy_I', 'Val_Sell_R', 'Val_Sell_I']]
            df_RI_tab['Date'] = df_RI_tab['Date'].apply(lambda x: str(x))
            cols = ['No_Buy_R', 'No_Buy_I', 'No_Sell_R', 'No_Sell_I', 'Vol_Buy_R', 'Vol_Buy_I', 'Vol_Sell_R',
                    'Vol_Sell_I', 'Val_Buy_R', 'Val_Buy_I', 'Val_Sell_R', 'Val_Sell_I']
            df_RI_tab[cols] = df_RI_tab[cols].astype('int64')
        else:
            ...
            # define columns
            columns = ['Date', 'No_Buy_R', 'No_Buy_I', 'No_Sell_R', 'No_Sell_I', 'Vol_Buy_R', 'Vol_Buy_I', 'Vol_Sell_R',
                       'Vol_Sell_I', 'Val_Buy_R', 'Val_Buy_I', 'Val_Sell_R', 'Val_Sell_I']
            # split data into defined columns
    ...
    for index, row in (ticker_no_df.reset_index()).iterrows():
        try:
            df_temp = get_ri_data(ticker_no=row['WEB-ID'], ticker=row['Ticker'], name=row['Name'],
                                  data_part=row['Market'])
            df_RI_tab = pd.concat([df_RI_tab, df_temp])
        except:
            pass
...
def Get_CWI_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
...
def Get_EWI_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=True,
                    show_weekday=False, double_date=False):
    """
...
def __Get_Day_IntradayTrades__(ticker_no, j_date):
    ...
    df_intraday['Time'] = df_intraday['Time'].astype('str').apply(
        lambda x: '0' + x[0] + ':' + x[1:3] + ':' + x[3:] if len(x) == 5 else x[:2] + ':' + x[2:4] + ':' + x[4:])
...
def Get_IntradayTrades_History(stock='وخارزم', start_date='1400-09-15', end_date='1400-12-29', jalali_date=True,
                               combined_datatime=False, show_progress=True):
    """
    ...
    if (no_days == 0):
        ...
        for j_date in j_date_list:
            try:
                df_intraday = pd.concat(
                    [df_intraday, __Get_Day_IntradayTrades__(ticker_no_list[day_counter - 1], j_date)], axis=0)
            except:
    ...
    if (jalali_date):
        if (combined_datatime):
            ...
            df_intraday['Date'] = df_intraday['J-Date'].apply(
                lambda x: jdatetime.date(int(x[:4]), int(x[5:7]), int(x[8:])).togregorian())
            df_intraday['DateTime'] = pd.to_datetime(
                df_intraday['Date'].astype(str) + ' ' + df_intraday['Time'].astype(str))
            print('Combining Jalali date and time takes a few more seconds!')
            df_intraday['J-DateTime'] = df_intraday['DateTime'].apply(
                lambda x: str(jdatetime.datetime.fromgregorian(datetime=x)))
...
def Get_USD_RIAL(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, show_weekday=False,
                 double_date=False):
    # check date validity --------------------------------------------------------------------------------------------------------------
    if (not ignore_date):
        start_date = __Check_JDate_Validity__(start_date, key_word="'START'")
    ...
    df_data = r.json()
    try:
        df_data = pd.DataFrame({'Date': df_data['t'], 'Open': df_data['o'], 'High': df_data['h'], 'Low': df_data['l'],
                                'Close': df_data['c'], })
...
def __Get_Day_MarketClose_BQ_SQ__(ticker_no, j_date):
    ...
    while (time > 122900):
        ...
        if (len(end_lob) == 0):  # go one second back and try again
            ...
            break
    df_sq_bq = pd.DataFrame({'J-Date': [j_date], 'Day_UL': [int(day_lb)], 'Day_LL': [int(day_ub)],
                             'Time': [str(time)[:2] + ':' + str(time)[2:4] + ':' + str(time)[-2:]], \
                             'BQ_Value': [int(bq)], 'SQ_Value': [int(sq)], 'BQPC': [int(round(bq_percap, 0))],
...
def Get_Queue_History(stock='وخارزم', start_date='1400-09-15', end_date='1400-12-29', show_per_capita=True,
    ...
    if (no_days == 0):
        ...
        df_bq_sq_val = pd.DataFrame(
            columns=['J-Date', 'Day_UL', 'Day_LL', 'Time', 'BQ_Value', 'SQ_Value', 'BQPC', 'SQPC']).set_index(
            ['J-Date'])
        day_counter = 1
        for j_date in j_date_list:
            try:
                df_bq_sq_val = pd.concat(
                    [df_bq_sq_val, __Get_Day_MarketClose_BQ_SQ__(ticker_no_list[day_counter - 1], j_date)], axis=0)
            except:
                failed_jdates.append(j_date)
            if (show_progress):
                clear_output(wait=True)
    ...
    df_bq_sq_val = df_bq_sq_val[
        ['Date', 'Weekday', 'Day_UL', 'Day_LL', 'Value', 'Time', 'BQ_Value', 'SQ_Value', 'BQPC', 'SQPC']]
...
def __Get_Day_LOB__(ticker_no, j_date):
    ...
    data['Time'] = data['Time'].astype('str').apply(
        lambda x: datetime.time(hour=int(x[0]), minute=int(x[1:3]), second=int(x[3:])) if len(x) == 5 \
            else datetime.time(hour=int(x[:2]), minute=int(x[2:4]), second=int(x[4:])))
    data['Day_UL'] = day_ub
    data['Day_LL'] = day_lb
    data = data[
        ['J-Date', 'Time', 'Depth', 'Sell_No', 'Sell_Vol', 'Sell_Price', 'Buy_Price', 'Buy_Vol', 'Buy_No', 'Day_LL',
         'Day_UL', 'Date']]
    data = data.set_index(['J-Date', 'Time', 'Depth'])
    return data


def Get_IntradayOB_History(stock='کرمان', start_date='1400-08-01', end_date='1400-08-01', jalali_date=True,
                           combined_datatime=False, show_progress=True):
    """
    ...
    if (no_days == 0):
        ...
        df_lob = pd.DataFrame(
            columns=['J-Date', 'Time', 'Depth', 'Sell_No', 'Sell_Vol', 'Sell_Price', 'Buy_Price', 'Buy_Vol', 'Buy_No', \
...
def Get_CWPI_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
                     show_weekday=False, double_date=False):
    """
...
def Get_FFI_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
                    show_weekday=False, double_date=False):
    """
...
def Get_MKT1I_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
                      show_weekday=False, double_date=False):
    """
...
def Get_MKT2I_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
                      show_weekday=False, double_date=False):
    """
...
def Get_INDI_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
                     show_weekday=False, double_date=False):
    """
...
def Get_LCI30_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
                      show_weekday=False, double_date=False):
    """
...
def Get_ACT50_History(start_date='1395-01-01', end_date='1400-12-29', ignore_date=False, just_adj_close=False,
                      show_weekday=False, double_date=False):
    """
...
def Get_MarketWatch():
    ...
    Mkt_RI_df.columns = ['WEB-ID', 'No_Buy_R', 'No_Buy_I', 'Vol_Buy_R', 'Vol_Buy_I', 'No_Sell_R', 'No_Sell_I',
                         'Vol_Sell_R', 'Vol_Sell_I']
    ...
    Mkt_RI_df = Mkt_RI_df[
        ['No_Buy_R', 'No_Buy_I', 'No_Sell_R', 'No_Sell_I', 'Vol_Buy_R', 'Vol_Buy_I', 'Vol_Sell_R', 'Vol_Sell_I']]
    # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Mkt_df.columns = ['WEB-ID', 'Ticker-Code', 'Ticker', 'Name', 'Time', 'Open', 'Final', 'Close', 'No', 'Volume',
                      'Value',
    ...
    Mkt_df['Market'] = Mkt_df['Mkt-ID'].map(
        {'300': 'بورس', '303': 'فرابورس', '305': 'صندوق قابل معامله', '309': 'پایه', '400': 'حق تقدم بورس',
         '403': 'حق تقدم فرابورس', '404': 'حق تقدم پایه'})
    ...
    cols = ['Open', 'Final', 'Close', 'No', 'Volume', 'Value', 'Low', 'High', 'Y-Final', 'EPS', 'Base-Vol', 'Day_UL',
            'Day_LL', 'Share-No']
    ...
    bq_value = Mkt_df.apply(lambda x: int(x['Buy-Vol'] * x['Buy-Price']) if (x['Buy-Price'] == x['Day_UL']) else 0,
                            axis=1)
    sq_value = Mkt_df.apply(lambda x: int(x['Sell-Vol'] * x['Sell-Price']) if (x['Sell-Price'] == x['Day_LL']) else 0,
                            axis=1)
    Mkt_df = pd.concat(
        [Mkt_df, pd.DataFrame(bq_value, columns=['BQ-Value']), pd.DataFrame(sq_value, columns=['SQ-Value'])], axis=1)
    # calculate buy/sell queue average per-capita:
    bq_pc_avg = Mkt_df.apply(
        lambda x: int(round(x['BQ-Value'] / x['Buy-No'], 0)) if ((x['BQ-Value'] != 0) and (x['Buy-No'] != 0)) else 0,
        axis=1)
    sq_pc_avg = Mkt_df.apply(
        lambda x: int(round(x['SQ-Value'] / x['Sell-No'], 0)) if ((x['SQ-Value'] != 0) and (x['Sell-No'] != 0)) else 0,
        axis=1)
    Mkt_df = pd.concat([Mkt_df, pd.DataFrame(bq_pc_avg, columns=['BQPC']), pd.DataFrame(sq_pc_avg, columns=['SQPC'])],
                       axis=1)
    ...
    final_df['Trade Type'] = pd.DataFrame(
        final_df['Ticker'].apply(lambda x: 'تابلو' if ((not x[-1].isdigit()) or (x in ['انرژی1', 'انرژی2', 'انرژی3']))
        else (
            'بلوکی' if (x[-1] == '2') else ('عمده' if (x[-1] == '4') else ('جبرانی' if (x[-1] == '3') else 'تابلو')))))
    ...
    final_df = final_df[
        ['Ticker', 'Trade Type', 'Time', 'Open', 'High', 'Low', 'Close', 'Final', 'Close(%)', 'Final(%)',
         'Day_UL', 'Day_LL', 'Value', 'BQ-Value', 'SQ-Value', 'BQPC', 'SQPC',
         'Volume', 'Vol_Buy_R', 'Vol_Buy_I', 'Vol_Sell_R', 'Vol_Sell_I', 'No', 'No_Buy_R', 'No_Buy_I', 'No_Sell_R',
         'No_Sell_I',
...
def Build_Market_StockList(bourse=True, farabourse=True, payeh=True, detailed_list=True, show_progress=True,):
    ...
    if (payeh):
        ...
        payeh_lookup = payeh_lookup[
            payeh_lookup['Market'].isin(['تابلو پایه زرد', 'تابلو پایه نارنجی', 'تابلو پایه قرمز'])]
    ...
    if (not detailed_list):
        ...
        df_final.columns = ['Ticker(12)', 'Ticker(5)', 'Name(EN)', 'Ticker(4)', 'Name', 'Comment', 'Ticker(30)',
                            'Company Code(12)',
                            'Panel', 'Panel Code', 'Sector Code', 'Sector', 'Sub-Sector Code', 'Sub-Sector', 'Ticker',
                            'Market', 'WEB-ID']
        ...
        df_final = df_final[['Ticker', 'Name', 'Market', 'Panel', 'Sector', 'Sub-Sector', 'Comment', 'Name(EN)', \
                             ...
                             'Sub-Sector Code', 'Panel Code', 'WEB-ID']]
        ...
        df_final.rename(columns={"WEB-ID":'inscode'},inplace=True)
    ...
    def find_code(stock_list):
        ...
        async def parallel_request(stock_list):
            ...
            async def get_data(session, stock):
                ...
                async with session.get(url, headers=headers) as response:
                    ...
                    data['Active'] = pd.to_numeric(data['Active'])
                    data = data.sort_values('Ticker')
                    data = pd.DataFrame(data[['Name', 'WEB-ID', 'Name-Split', 'Symbol-Split']].values,
                                        columns=['Name', 'WEB-ID',
                                                 'Name-Split', 'Symbol-Split'],
                                        index=pd.MultiIndex.from_frame(data[['Ticker', 'Active']]))
        ...
        def request(stock_list):
            ...
            for stock in stock_list:
                while True:
                    try:
                        data_id = requests.get(f'http://old.tsetmc.com/tsev2/data/search.aspx?skey={stock}',
        ...
        if len(df_names) > 0:
            ...
            stock_list = [characters.fa_to_ar(''.join(i.split('\u200c')).strip()) for i in
                          df_names.index[~df_names.index.get_level_values('Ticker').duplicated()].get_level_values(
                              'Ticker')]
...
def __build_price_panel_seg__(df_response, param):
    ...
    for ticker, ticker_no_df in df_response.groupby(level=0):
        ...
        # loop to get data from different pages of a ticker:
        for index, row in (ticker_no_df.reset_index()).iterrows():
            df_temp = __process_price_data__(ticker_no=row['WEB-ID'], ticker=row['Ticker'], r=row['price'],
                                             data_part=index + 1)
            df_history = pd.concat([df_history, df_temp])
        # sort index and reverse the order for more processes:
        df_history = df_history.sort_index(ascending=True)
        ...
        # Y-Final for new part of data could be 0 or 1000, we need to replace them with yesterday's final price:
        df_history['Final(+1)'] = df_history['Final'].shift(+1)  # final prices shifted forward by one day
        df_history['Part(+1)'] = df_history['Part'].shift(+1)  # market shifted forward by one day
        df_history['temp'] = df_history.apply(lambda x: x['Y-Final'] if ((x['Y-Final'] != 0) and (x['Y-Final'] != 1000))
        else (x['Y-Final'] if ((x['Part(+1)'] == x['Part']) or (pd.isnull(x['Final(+1)'])))
              else x['Final(+1)']), axis=1)
        df_history['Y-Final'] = df_history['temp']
...
def Get_60D_PriceHistory(stock_list, adjust_price=True, show_progress=True):
    ...
    r_trading_days = requests.get(f'http://cdn.tsetmc.com/api/Index/GetIndexB2History/32097828799138957',
                                  headers=headers)
        ...
        for ticker in available_stocks_list:
            ...
            if ((ticker_60D_hist_df['Y-Final'][0] == 0) or (ticker_60D_hist_df['Y-Final'][0] == 100)):
                ...
                ticker_60D_hist_df['COEF'] = (
                        ticker_60D_hist_df['Y-Final'].shift(-1) / ticker_60D_hist_df['Final']).fillna(1.0)
                ticker_60D_hist_df['ADJ-COEF'] = ticker_60D_hist_df.iloc[::-1]['COEF'].cumprod().iloc[::-1]
                ticker_60D_hist_df['Adj Open'] = (ticker_60D_hist_df['Open'] * ticker_60D_hist_df['ADJ-COEF']).apply(
                    lambda x: int(x))
                ticker_60D_hist_df['Adj High'] = (ticker_60D_hist_df['High'] * ticker_60D_hist_df['ADJ-COEF']).apply(
                    lambda x: int(x))
                ticker_60D_hist_df['Adj Low'] = (ticker_60D_hist_df['Low'] * ticker_60D_hist_df['ADJ-COEF']).apply(
                    lambda x: int(x))
                ticker_60D_hist_df['Adj Close'] = (ticker_60D_hist_df['Close'] * ticker_60D_hist_df['ADJ-COEF']).apply(
                    lambda x: int(x))
                ticker_60D_hist_df['Adj Final'] = (ticker_60D_hist_df['Final'] * ticker_60D_hist_df['ADJ-COEF']).apply(
                    lambda x: int(x))
                ...
                ticker_adj_info_df = pd.DataFrame(
                    {'Ticker': [ticker], 'ADJ-JDate': [ticker_adj_jdate], 'ADJ-Coef': [ticker_adj_coeff]})
...
def Get_Queue_History(stock = 'فولاد', start_date = '1300-09-15', end_date='1403-12-29', show_per_capita = True,double_date = False, show_progress = True):
    """
    دریافت ارزش صف خرید یا فروش یک سهم در زمان بسته شدن بازار، در روزهای معاملاتی بین تاریخ شروع و پایان
    اگر فقط به داده های یک روز مشخص نیاز دارید از تاریخ شروع و پایان یکسان استفاده کنید
    """
    ...
    for index, row in (ticker_no_df.reset_index()).iterrows():
        try:
            df_temp = get_price_data_forintraday(ticker_no = row['WebID'])
            df_history = pd.concat([df_history,df_temp])
        except:
            pass
    # sort index and reverse the order for more processes:
    ...
    if(no_days==0):
        ...
        for j_date in j_date_list:
            ...
            if (show_progress):
                ...
                print('Progress : ', f'{round((day_counter) / no_days * 100, 1)} %')
            day_counter += 1
            df_bq_sq_val = pd.concat([df_bq_sq_val, df_history[['Value', 'Date']]], axis=1, join="inner")
            cols = ['Day_UL', 'Day_LL', 'Value', 'BQ_Value', 'SQ_Value', 'BQPC', 'SQPC']
            df_bq_sq_val[cols] = df_bq_sq_val[cols].apply(pd.to_numeric).astype('int64')
            # re-arrange columns order:
            df_bq_sq_val = df_bq_sq_val[
                ['Date', 'Day_UL', 'Day_LL', 'Value', 'Time', 'BQ_Value', 'SQ_Value', 'BQPC', 'SQPC']]
            if (not show_per_capita):
                df_bq_sq_val.drop(columns=['BQPC', 'SQPC'], inplace=True)
            if (not double_date):
                df_bq_sq_val.drop(columns=['Date'], inplace=True)
